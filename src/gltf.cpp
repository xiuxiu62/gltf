...
Model Model::load_from_file(bool &success, const std::string &path) {
    Model model;

    usize last_slash = path.find_last_of("/\\");
    if (last_slash != std::string::npos) {
        model.base_path = path.substr(0, last_slash + 1); // Keep the trailing slash
    } else {
        model.base_path = ""; // No directory part, just the current directory
    }
...
// For uri parsing
std::vector<u8> Model::decode_base_64(const std::string &input) {
    // Define Base64 mapping table
    static const u8 decode_table[256] = {
